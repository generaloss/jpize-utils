...
public class ShortList implements Iterable<Short> {
    ...
    public void addAll(ShortBuffer buffer) {
    ...
    public void addAll(Collection<Short> collection) {
        for(Short item: collection)
            this.add(item);
    }

    public <T> void addAll(Iterable<T> iterable, Function<T, Short> func) {
        for(T object: iterable)
            this.add(func.apply(object));
    }

    public <T> void addAll(Collection<T> collection, Function<T, Short> func) {
        for(T object: collection)
            this.add(func.apply(object));
    }

    public <T> void addAll(T[] array, Function<T, Short> func) {
        for(T object: array)
            this.add(func.apply(object));
    }
    ...
    public void removeFirst(short value) {
        final int index = indexOf(value);
        if(index > -1)
            remove(index);
    }

    public void removeLast(short value) {
